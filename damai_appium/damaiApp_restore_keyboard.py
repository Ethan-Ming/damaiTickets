...
# Store the original keyboard state
original_keyboard_state = device_app_info.get_capability('unicodeKeyboard')

def reset_keyboard(driver):
    try:
        driver.set_capability('unicodeKeyboard', original_keyboard_state)
        driver.set_capability('resetKeyboard', False)
        print("Keyboard reset to original state.")
    except Exception as e:
        print(f"Failed to reset keyboard: {e}")

def signal_handler(sig, frame):
    print('You pressed Ctrl+C!')
    reset_keyboard(driver)
    driver.quit()
    sys.exit(0)
